...
def ensure_three_channels(x: torch.Tensor) -> torch.Tensor:
    ...
    LPIPS-style perceptual nets are RGB (3ch). If grayscale, repeat.
    """
    if x.shape[1] == 1:
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="./outputs/7t-restormer-lpips-new")
    ...
    l1 = nn.L1Loss()
    percep2d = PerceptualLoss(
    spatial_dims=2,
    network_type="vgg",     # "alex", "vgg", or "squeeze"
    pretrained=True,        # LPIPS weights
    ).to(device=device)
    percep2d.eval()  
    alpha = 0.1

    def criterion(pred: torch.Tensor, targ: torch.Tensor) -> torch.Tensor:
        """
        pred/targ: [B, 1 or 3, H, W] in [0,1] (or call to_unit_range first)
        """
        p = ensure_three_channels(pred)
        t = ensure_three_channels(targ)
        lp = percep2d(p, t)         # LPIPS-style perceptual loss
        l  = l1(pred, targ)         # pixel fidelity at native resolution
        return alpha * lp + (1.0 - alpha) * l
