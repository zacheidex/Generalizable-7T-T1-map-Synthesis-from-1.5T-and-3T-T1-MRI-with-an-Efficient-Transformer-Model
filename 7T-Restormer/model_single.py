...
class TLP(nn.Module):
    """
    Transformer-based MRI synthesis without fuzzy prompt generation.
    Accepts a single-channel input (e.g. T1) and produces output (e.g. T1ce).
    """
    def __init__(
        self,
        inp_A_channels=1,
        ...
        # Single-branch encoder (no prompt concatenation)
    ...
    def forward(self, inp_img_A):
        # Encode
        enc3, enc2, enc1 = self.encoder_A(inp_img_A)
        # Spatial attention + decode
        x3 = self.attention_spatial(enc3)
        d3 = self.decoder_level3(x3)
        u2 = self.up3_2(d3)
        cat2 = torch.cat([u2, enc2], dim=1)
        r2 = self.reduce_chan_level2(cat2)
        d2 = self.decoder_level2(r2)
        u1 = self.up2_1(d2)
        cat1 = torch.cat([u1, enc1], dim=1)
        d1 = self.decoder_level1(cat1)
        ref = self.refinement(d1)
        out = self.output(ref)
...
class Encoder_A(nn.Module):
    def __init__(self, inp_channels=2, dim=48, num_blocks=(1, 2, 2), heads=(2, 4, 8), ffn_expansion_factor=2,
                 bias=False,
                 LayerNorm_type='WithBias'):
